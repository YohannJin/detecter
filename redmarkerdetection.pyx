...
def marker_detection(np.ndarray[DTYPE_t, ndim=3] frame,np.ndarray[DTYPE_t, ndim=1] seg_papram):
    ...
    for i in range(len(quads_prj)):
        ...
        if quads_ID[i] == 0:
            ...
            cv2.putText(frame, '6', (bbox[0], bbox[1]), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (255, 255, 255), 2)
        elif quads_ID[i] == 6:
            cv2.putText(frame, '7', (bbox[0], bbox[1]), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (255, 255, 255), 2)
        elif quads_ID[i] == 7:
            cv2.putText(frame, '8', (bbox[0], bbox[1]), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (255, 255, 255), 2)

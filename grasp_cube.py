...
class graspAruco:
    def __init__(self):
        ...
        self.grasp_state_pub = rospy.Publisher("action_state", int)

        self.image_sub = rospy.Subscriber("/aruco_pose", Pose, self.graspCallback, queue_size = 1)
        self.action_require_sub = rospy.Subscriber("/require_action", int , self.action_required_callback, queue_size = 1)
    ...
    def graspCallback(self,data):
        ...
        if action_required == 1:
            ...
            if (distance_in_x <= gama_x) and (distance_in_y <= gama_y):
                ...
                action_required = 0
                self.grasp_state_pub.publish(1)
    ...
    def action_required_callback(self, require):
        if require == 1:
            action_required = 1
        else: 
            action_required = 0
